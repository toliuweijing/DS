  ...
  : port_(p1), counting_(count), curr_counts_(count), lossytest_(0), reachable_ (true)
{
	...
	VERIFY(pthread_mutex_init(&mutexForgotten, 0) == 0); // @Junyang
...
{
	ScopedLock rwl(&reply_window_m_);
  ScopedLock lockForgottens(&mutexForgotten);
  enum requestType {newReq, inpro, done, forgotten};
  requestType result = newReq;
  ...
  if (reply_window_.find(clt_nonce) != reply_window_.end()) {
    ...
    // Check the forgotten map.
    if (result == newReq) {
      if(theForgottens.find(clt_nonce) != theForgottens.end()) {
        std::list<unsigned int>::iterator forgotIt;
        for (forgotIt = theForgottens[clt_nonce].begin();
            forgotIt != theForgottens[clt_nonce].end(); ++forgotIt) {
          if((*forgotIt) == xid) {
            result = forgotten;
            break;
          }
    ...
    // If result is still newReq, then the request is indeed new!
    if (result == newReq) {
      reply_t newReqEntry(xid);
      reply_window_[clt_nonce].push_back(newReqEntry);
    }
  } else {
    reply_t newReqEntry(xid);
    std::list<reply_t> lst;
    reply_window_[clt_nonce] = lst;
    reply_window_[clt_nonce].push_back(newReqEntry);
  }

  // Trim reply window proplerly.
  for (it = reply_window_[clt_nonce].begin();
      it != reply_window_[clt_nonce].end();) {
    if (((*it).xid) <= xid_rep) {
      if(theForgottens.find(clt_nonce) == theForgottens.end())
        theForgottens[clt_nonce] = std::list<unsigned int>();
      theForgottens[clt_nonce].push_back((*it).xid);
      free((*it).buf);
