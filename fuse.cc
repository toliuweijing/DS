                   ...
                   int to_set, struct fuse_file_info *fi)
{
  ...
  if (FUSE_SET_ATTR_SIZE & to_set) {
    ...
    yfs_client::status ret;
    yfs_client::inum fileInum = ino;
    ret = yfs->setSize(fileInum, attr->st_size);
    if (ret == yfs_client::OK) {
      getattr(fileInum, st);
      fuse_reply_attr(req, &st, 0);
    } else {
                ...
                off_t off, struct fuse_file_info *fi)
{
  ...
  yfs_client::status ret;
  yfs_client::inum fileInum = ino;
  ret = yfs->readFile(fileInum, off, size, buf);
  if (ret == yfs_client::OK) {
                 ...
                 struct fuse_file_info *fi)
{
  ...
  ret = yfs->writeFile(fileInum, off, size, str.substr(0, size));
  if (ret == yfs_client::OK) {
                        ...
                        mode_t mode, struct fuse_entry_param *e)
{
  ...
  bool found = false;
  std::cout << "createhelper parentInum:" << parentInum << " name:" << name
      << std::endl; 
  ret = yfs->lookup(name, parentInum, ino, found, false);
  //std::cout << "here: ret is " << ret << std::endl;
  if (ret == yfs_client::OK) {
    // yfs->lookup returns yfs_client::OK means that the parent directory
    // does exist.
    if (found) {
      std::cout << "createhelper called on parentIum:" << parentInum << " name:"
          << name << " got EXISIT error" << std::endl;
      yfs->releaseLock(parentInum);
      return yfs_client::EXIST;
    } else {
      // Generate a random unique ino for the file;
      ino = random();
      while (ino == 0x00000001) {
        ino = random();
      }
      
      // The 31st bit of a file's i-number should be 1.
      yfs_client::inum mask = 1;
      mask = (mask << 32) - 1;
      ino = ino & mask;
      mask = 1;
      mask = mask << 31;
      ino = ino | mask;
      // Create the new file. Function yfs->addDirent does both adding
      // an directory entry for the new file and create the new file.
      ret = yfs->addDirent(parentInum, ino, name);
      if (ret == yfs_client::OK) {
        e->ino = ino;
        getattr(ino, e->attr);
        return yfs_client::OK;
    ...
    // yfs->lookup returns yfs_client::NOENT when the parent directory doesn't
    // exist, so this function just forwards it.
    return ret;
  }
}
...
fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  yfs_client::status ret;
  yfs_client::inum parentInum = parent;
  yfs_client::inum childInum;
  std::stringstream logSs;
  std::cout << "caution: filename is originally " << name << std::endl;
  logSs << "lookup " << std::string(name) << " in inum "
      << parentInum << std::endl;
  std::cout << logSs.str();
  ret = yfs->lookup(name, parentInum, childInum, found, true);
  if (ret != yfs_client::OK) {
    // This means even the parent directory doesn't exist at all.
    fuse_reply_err(req, ENOENT);
  } else {
    if (found) {
      e.ino = childInum;
      getattr(childInum, e.attr);
      fuse_reply_entry(req, &e);
    }
    else
      fuse_reply_err(req, ENOENT);
                   ...
                   off_t off, struct fuse_file_info *fi)
{
  yfs_client::inum inum = ino; // req->in.h.nodeid;
  struct dirbuf b;
  std::cout << "fuseserver_readdir is called on inum " << ino << std::endl;
     ...
     mode_t mode)
{
  ...
  yfs_client::inum parentInum = parent;
  bool found = false;
  std::cout << "mkdir parentInum:" << parentInum << " name:" << name
      << std::endl; 
  ret = yfs->lookup(name, parentInum, ino, found, false);
  //std::cout << "here: ret is " << ret << std::endl;
  if (ret == yfs_client::OK) {
    ...
    // does exist.
    if (found) {
      std::cout << "mkdir called on parentIum:" << parentInum << " name:"
          << name << " got EXISIT error" << std::endl;
      yfs->releaseLock(parentInum);
			fuse_reply_err(req, EEXIST);
    } else {
      ...
      // The 31st bit of a directory's i-number should be 0.
      yfs_client::inum mask = 1;
      mask = (mask << 31) - 1;
      ino = ino & mask;
      // Create the new directory. Function yfs->addDirent does both adding
      // an directory entry for the new directory and create the new directory.
      ret = yfs->addDirent(parentInum, ino, name);
      if (ret == yfs_client::OK) {
        e.ino = ino;
        getattr(ino, e.attr);
    ...
    // yfs->lookup returns yfs_client::NOENT when the parent directory doesn't
    // exist, so this function should reply with an error.
    fuse_reply_err(req, ENOSYS);
  }
#else
  fuse_reply_err(req, ENOSYS);
...
fuseserver_unlink(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  yfs_client::inum parentInum = parent;
  bool found = false;
  std::cout << "unlink parentInum:" << parentInum << " name:" << name
      << std::endl; 
  ret = yfs->lookup(name, parentInum, ino, found, false);
  //std::cout << "here: ret is " << ret << std::endl;
  if (ret == yfs_client::OK) {
    ...
    if (found) {
      if (!yfs->isfile(ino)) {
        ...
        ret = yfs->unlink(parentInum, ino, name);
        if (ret == yfs_client::OK) {
